...
void  SetupOledPix () { 
  ...
  Clear();
  // DK add because it said to for 32 height displays
  //Cmd (0xDA);
  //Cmd (0x02);
}
void  WrStaData () {  
...
// ===========================
void vCenterLine (byte xx, byte h) { // DK addition
  if(h>16) { h=16; }

  // Draw bottom half of display
  for (byte i=0; i<h; i+=4) { // (4 pixels per byte on 128x32 display)
    byte bloc = 0xFF;
    switch(h-i) { // If loop is on end of line segment, draw partial block
      case 1:
      bloc = 0x06;
      break;
      case 2:
  ...
  // Draw top half of display
  for (byte i=16; i>16-h; i-=4) { // (4 pixels per byte on 128x32 display)
    byte bloc = 0xFF;
    switch(h+i-16) { // If loop is on start of line segment, draw partial block
      case 3:
      bloc = 0xF8;
      break;
      case 2:
...
void vLineErase (byte xx) { // DK addition
  for (byte i=0; i<8; i++) { 
    LiCol (i,xx); 
    Start(); Write (Adr); Write (0x40);
    Write (0); Stop();
